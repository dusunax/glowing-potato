...
export function RoomLobby({ rooms, onJoinRoom, onJoinPrivate, onCreateRoom, onBack, nickname, onSignOut, onUpdateNickname }) {
    ...
    const joinPrivateInputRef = useRef(null);
    ...
    return (<div className="min-h-screen flex flex-col p-6 bg-gp-bg">
      {/* Header */}
      <header className="mb-8">
        <div className="mb-4 flex items-center justify-between">
          ...
          <button type="button" onClick={() => setShowNicknamePopup(true)} aria-label="Edit nickname" className="w-8 h-8 rounded-full border border-gp-accent/30 text-gp-mint/70 hover:text-gp-mint hover:border-gp-accent/60 hover:bg-gp-accent/10 flex items-center justify-center transition-colors" title={`Nickname: ${playerName || 'You'}`}>
            ✎
          </button>
          {nickname && onSignOut && (<button type="button" onClick={onSignOut} className="px-2 py-1.5 rounded-lg border border-gp-accent/20 text-gp-mint/50 hover:text-gp-mint/80 hover:border-gp-accent/40 transition-colors text-xs">
              Sign out
            </button>)}
          </div>
        </div>
      ...
      <div className="max-w-2xl mx-auto w-full space-y-6">
        ...
        {showNicknamePopup && nickname && onUpdateNickname && (<UserEditPopup currentNickname={nickname} onSave={onUpdateNickname} onClose={closeNicknamePopup}/>)}
        {showNicknamePopup && !nickname && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 backdrop-blur-sm p-4" onClick={closeNicknamePopup}>
            <Card className="w-full max-w-sm p-5 space-y-4" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={closeNicknamePopup} className="text-gp-mint/70 hover:text-gp-mint text-lg leading-none" aria-label="Close">
                  ✕
                </button>
              </div>
              <div className="space-y-3">
                <label className="block text-xs text-gp-mint/70 mb-1">Your Nickname</label>
                <input type="text" value={draftPlayerName} ref={nicknameInputRef} onChange={(e) => setDraftPlayerName(e.target.value)} placeholder="You" maxLength={18} className="w-full bg-gp-bg border border-gp-accent/40 rounded-lg px-3 py-2 text-sm text-gp-mint placeholder:text-gp-mint/30 focus:outline-none focus:ring-2 focus:ring-gp-mint/40" onKeyDown={(e) => e.key === 'Enter' && saveNickname()}/>
              </div>
        ...
        {/* Create room panel */}
        {/* Create room panel */}
        {showCreate && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 backdrop-blur-sm p-4" onClick={closeCreateRoom}>
            <Card className="w-full max-w-md p-5 space-y-4" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={closeCreateRoom} className="text-gp-mint/70 hover:text-gp-mint text-lg leading-none" aria-label="Close">
                  ✕
                </button>
              </div>
              <div className="space-y-3">
                <div>
                  ...
                  <input type="text" value={newTitle} ref={createTitleRef} onChange={(e) => setNewTitle(e.target.value)} placeholder="e.g. Game Night 🎉" maxLength={40} className="w-full bg-gp-bg border border-gp-accent/40 rounded-lg px-3 py-2 text-sm text-gp-mint placeholder:text-gp-mint/30 focus:outline-none focus:ring-2 focus:ring-gp-mint/40" onKeyDown={(e) => e.key === 'Enter' && handleCreate()}/>
                </div>
                <div>
                  <label className="block text-xs text-gp-mint/70 mb-2">Visibility</label>
                  <div className="flex gap-3">
                    {['public', 'private'].map((v) => (<button key={v} onClick={() => setNewVisibility(v)} className={`flex-1 py-2 rounded-lg text-sm font-medium border transition-colors ${newVisibility === v
                ...
                <div>
                  ...
                  <div className="flex gap-3">
                    {[2, 3, 4].map((size) => (<button key={size} onClick={() => setMaxPlayers(size)} className={`flex-1 py-2 rounded-lg text-sm font-medium border transition-colors ${maxPlayers === size
                    ? 'bg-gp-accent/20 border-gp-accent text-gp-mint'
                    : 'bg-transparent border-gp-accent/30 text-gp-mint/60 hover:border-gp-accent/60'}`}>
                        {size} Players
                      </button>))}
        ...
        {showJoinPrivate && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/45 backdrop-blur-sm p-4" onClick={closeJoinPrivate}>
            <Card className="w-full max-w-md p-5 space-y-4" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between">
                <h2 className="text-gp-mint font-semibold text-base">Join Private Room</h2>
                <button type="button" onClick={closeJoinPrivate} className="text-gp-mint/70 hover:text-gp-mint text-lg leading-none" aria-label="Close">
                  ✕
                </button>
              </div>
              <div className="space-y-3">
                <div>
                  ...
                  <input type="text" value={privateId} ref={joinPrivateInputRef} onChange={(e) => { setPrivateId(e.target.value.toUpperCase()); setPrivateError(''); }} placeholder="e.g. ABC123" maxLength={6} className="w-full bg-gp-bg border border-gp-accent/40 rounded-lg px-3 py-2 text-sm text-gp-mint placeholder:text-gp-mint/30 focus:outline-none focus:ring-2 focus:ring-gp-mint/40 tracking-widest uppercase" onKeyDown={(e) => e.key === 'Enter' && handleJoinPrivate()}/>
                  {privateError && (<p className="text-red-400 text-xs mt-1">{privateError}</p>)}
                </div>
                <div className="flex gap-2 justify-end">
        ...
        <div>
          ...
          <div className="space-y-2">
            {publicSlots.map((room, idx) => (room ? (<RoomRow key={room.id} room={room} onJoin={() => onJoinRoom(room.id, playerName)}/>) : (<RoomRow key={`empty-slot-${idx}`} room={null}/>)))}
          </div>
        </div>
...
function RoomRow({ room, onJoin }) {
    if (!room) {
        return (<div className="flex items-center gap-3 p-3 rounded-xl bg-gp-surface/20 border border-dashed border-gp-accent/20">
    ...
    return (<div className="flex items-center gap-3 p-3 rounded-xl bg-gp-surface/50 border border-gp-accent/20 hover:border-gp-accent/50 transition-colors">
      ...
      <Button variant={full ? 'ghost' : 'secondary'} size="sm" onClick={onJoin} disabled={full}>
        {full ? 'Full' : 'Join'}
      </Button>
    </div>);
}
