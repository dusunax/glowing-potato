...
export function GameRoom({ game, onLeave, onSendMessage, onStartGame, onCastVote, sttSupported, sttActive, sttError, onToggleStt, sttInterim, onSignOut, }) {
    ...
    return (<div className="h-screen min-h-screen max-h-screen flex flex-col overflow-hidden bg-gp-bg">
      <>
        {/* Top bar */}
        <header className="flex items-center gap-3 px-4 py-3 border-b border-gp-accent/20 bg-gp-surface/30">
          {game.phase !== 'playing' && (<Button variant="ghost" size="sm" onClick={onLeave}>← Leave</Button>)}
          ...
          {privateRoomCode && (<Button variant="outline" size="sm" onClick={handleCopyRoomCode} title="Copy private room code" className="h-7 px-3 gap-1 border-gp-mint/50 text-gp-mint hover:bg-gp-accent/20">
              📋 {copied ? 'Copied!' : 'Copy room code'}
            </Button>)}
          {onSignOut && (<button type="button" onClick={onSignOut} className="px-2 py-1.5 rounded-lg border border-gp-accent/20 text-gp-mint/50 hover:text-gp-mint/80 hover:border-gp-accent/40 transition-colors text-xs">
              Sign out
            </button>)}
        </header>
        ...
        {game.phase === 'playing' && (<PlayingView game={game} onSendMessage={onSendMessage} sttSupported={sttSupported} sttActive={sttActive} sttError={sttError} onToggleStt={onToggleStt} sttInterim={sttInterim} onLeave={onLeave}/>)}
        {game.phase === 'finished' && (<FinishedView game={game} onLeave={onLeave} onShowReplay={() => setShowReplayModal(true)}/>)}
      </>
      {game.phase === 'finished' && showReplayModal && (<ChatReplayPage game={game} onClose={() => setShowReplayModal(false)}/>)}
    </div>);
}
// ---------------------------------------------------------------------------
// Waiting view
// ---------------------------------------------------------------------------
function WaitingView({ game, onStartGame }) {
    const isHost = game.isHost;
    const canStart = isHost && game.players.length >= 2;
    const maxPlayers = game.maxPlayers || 4;
    return (<div className="h-full min-h-0 flex-1 flex flex-col items-center justify-center gap-6 p-6 overflow-y-auto gp-scrollbar">
      ...
      {isHost && (<Button variant={canStart ? 'primary' : 'secondary'} size="md" onClick={onStartGame} disabled={!canStart}>
          {canStart ? '▶ Start Game' : 'Need at least 2 players'}
        </Button>)}
      {!isHost && <p className="text-gp-mint/50 text-xs">Waiting for the host to start.</p>}
      {/* Player list */}
      <div className="w-full max-w-xs space-y-2">
        {game.players.map((p) => (<div key={p.id} className="flex items-center gap-3 p-3 rounded-xl bg-gp-surface/50 border border-gp-accent/20">
            ...
            {game.isHost && p.id === game.localPlayerId && (<span className="text-sm" title="Room creator">
                👑
              </span>)}
          </div>))}
        {/* Empty slots */}
        {Array.from({ length: Math.max(0, maxPlayers - game.players.length) }).map((_, i) => (<div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-gp-surface/20 border border-dashed border-gp-accent/20">
...
function VotingView({ game, onCastVote }) {
    ...
    return (<div className="h-full min-h-0 flex-1 overflow-y-auto p-4 space-y-5 gp-scrollbar">
      {/* Countdown */}
      ...
      <div className="bg-gp-surface/30 border border-gp-accent/20 rounded-xl p-4">
        ...
        <div className="flex gap-2">
          {localPlayer.wordSlots.map((slot, si) => (<div key={si} className="flex-1 rounded-lg bg-gp-bg/60 border border-gp-accent/20 p-2 text-center">
      ...
      {/* Other players */}
      {otherPlayers.map((player) => (<div key={player.id} className="bg-gp-surface/50 border border-gp-accent/20 rounded-xl p-4">
          <div className="flex items-center gap-2 mb-3">
          ...
          <div className="space-y-3">
            {player.wordSlots.map((slot, si) => (<div key={si}>
                ...
                <div className="flex gap-2">
                  {slot.candidates.map((word, wi) => {
                    ...
                    return (<button key={wi} onClick={() => handleVote(player, si, wi)} className={`flex-1 py-2 px-1 rounded-lg border text-xs font-medium transition-all ${isMyVote
                    ...
                    <span className="block capitalize">{word}</span>
                        ...
                        {totalVotes > 0 && (<span className="block text-gp-mint/40 text-[10px] mt-0.5">{votePct}%</span>)}
                      </button>);
                })}
                </div>
              </div>))}
...
function VoteDots({ votes, highlightIndex }) {
    const total = votes.reduce((a, b) => a + b, 0);
    if (total === 0)
        return <span className="block h-2"/>;
    const display = Math.min(total, 8);
    return (<span className="flex justify-center gap-0.5 mt-1 h-2">
      {Array.from({ length: display }).map((_, i) => (<span key={i} className={`w-1.5 h-1.5 rounded-full ${highlightIndex !== undefined ? 'bg-gp-accent' : 'bg-gp-mint/40'}`}/>))}
    </span>);
}
function PlayingView({ game, onSendMessage, sttSupported, sttActive, sttError, onToggleStt, sttInterim, onLeave, }) {
    ...
    return (<div className="h-full min-h-0 flex-1 flex flex-col overflow-hidden">
      {/* Players bar */}
      <div className="flex gap-2 px-4 py-2 overflow-x-auto border-b border-gp-accent/20 bg-gp-surface/20">
        {game.players.map((p) => (<PlayerChip key={p.id} player={p} isLocal={p.id === game.localPlayerId}/>))}
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-3 space-y-2 pb-10 gp-scrollbar">
            {game.messages.length === 0 && (<p className="text-gp-mint/30 text-xs text-center pt-8">
                Game started! Chat freely — but don't say your forbidden words!
              </p>)}
            {game.messages.map((msg) => {
            const isLocal = msg.playerId === game.localPlayerId;
            return (<div key={msg.id} className={`flex flex-col ${isLocal ? 'items-end' : 'items-start'}`}>
                  <span className="text-gp-mint/50 text-[10px] px-1 mb-0.5">{msg.playerName}</span>
                  <div className={`px-3 py-1.5 rounded-xl text-sm max-w-[75%] break-words ${msg.triggeredWord
                  ...
                  {msg.triggeredWord && (<span className="text-red-400 text-xs mt-0.5 px-1">
                      ❌ Said &ldquo;{msg.triggeredWord}&rdquo; — OUT!
                    </span>)}
                </div>);
        })}
            {sttInterim && (<div className="flex flex-col items-end">
          ...
          <div className="p-3 border-t border-gp-accent/20 bg-gp-bg">
            {isOut ? (<div className="text-center text-red-400 text-sm py-2">
                ❌ You're out! Watch how the others do.
              </div>) : (<div className="space-y-1.5">
                {sttError && (<p className="text-red-400 text-xs px-1">{sttError}</p>)}
                <div className="flex gap-2 items-center">
                  <input type="text" value={inputText} onChange={(e) => setInputText(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSend()} placeholder="Type carefully…" className="flex-1 bg-gp-surface/40 border border-gp-accent/30 rounded-lg px-3 py-2 text-sm text-gp-mint placeholder:text-gp-mint/30 focus:outline-none focus:ring-2 focus:ring-gp-mint/40"/>
                  {sttSupported && (<Button variant={sttActive ? 'destructive' : 'secondary'} size="icon" onClick={onToggleStt} title={sttActive ? 'Stop speech recognition' : 'Start speech recognition'} className="flex-shrink-0">
                      {sttActive ? '🎙️' : '🎤'}
                    </Button>)}
        ...
        <aside className="hidden md:flex flex-col w-48 border-l border-gp-accent/20 bg-gp-surface/10 overflow-y-auto p-3 gap-3 gp-scrollbar">
          ...
          {game.players
            ...
            .map((p) => (<PlayerWordCard key={p.id} player={p}/>))}
      ...
      {showQuitConfirm && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/65 p-4" onClick={() => setShowQuitConfirm(false)}>
          <div className="w-full max-w-sm rounded-xl border border-gp-accent/30 bg-gp-surface p-4 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            ...
            <div className="flex gap-2 justify-end">
              <Button variant="outline" size="sm" onClick={() => setShowQuitConfirm(false)} className="border-gp-mint/70 text-gp-mint hover:bg-gp-mint/10">
                Cancel
              </Button>
...
function highlightWord(text, word) {
    ...
    return (<>
      {parts.map((part, i) => part.toLowerCase() === word.toLowerCase() ? (<mark key={i} className="bg-red-500/40 text-red-200 rounded px-0.5">
            {part}
          </mark>) : (part))}
    </>);
}
...
// ---------------------------------------------------------------------------
function PlayerChip({ player, isLocal }) {
    return (<div className={`flex-shrink-0 flex items-center gap-1.5 px-2 py-1 rounded-full text-xs font-medium border ${player.isOut
...
// ---------------------------------------------------------------------------
function PlayerWordCard({ player }) {
    const words = player.wordSlots.map((s) => s.finalWord).filter(Boolean);
    return (<div className={`rounded-lg p-2 border text-xs ${player.isOut
      ...
      {words.length > 0 ? (<div className="space-y-0.5">
          {words.map((w) => (<span key={w} className="block rounded px-1.5 py-0.5 bg-gp-accent/10 text-gp-mint capitalize">
              {w}
            </span>))}
        </div>) : (<span className="text-gp-mint/30">—</span>)}
    </div>);
}
// ---------------------------------------------------------------------------
// Finished view
// ---------------------------------------------------------------------------
function FinishedView({ game, onLeave, onShowReplay, }) {
    const localPlayer = game.players.find((p) => p.id === game.localPlayerId);
    const localName = localPlayer?.name ?? 'You';
    const winner = game.players.find((p) => p.id === game.winnerId && !p.isOut) ??
    ...
    function getResultText() {
        if (isLocalWinner)
            return `Congratulations ${localName} — you were the last one standing!`;
        if (winner)
            return `${winner.name} was the last one to avoid saying their forbidden words.`;
        return 'Everyone said their forbidden word. Well played all!';
    }
    return (<div className="flex-1 flex flex-col items-center justify-center gap-6 p-6 text-center">
      ...
      <div className="w-full max-w-2xl border border-gp-accent/30 rounded-xl bg-gp-surface/20 p-3">
        ...
        <div className="space-y-1.5">
          {ranking.map((entry) => {
            ...
            return (<div key={entry.player.id} className="bg-gp-surface/50 rounded-lg border border-gp-accent/20 px-3 py-2 text-sm flex items-center gap-2">
      ...
      <div className="flex gap-2">
        <Button variant="outline" size="lg" onClick={onShowReplay} title="Chat Replay" aria-label="Chat Replay">
...
function ChatReplayPage({ game, onClose }) {
    return (<div className="fixed inset-0 z-50 bg-black/70 p-3 flex items-center justify-center" onClick={onClose}>
      <div className="w-full max-w-5xl h-[85vh] bg-gp-bg border border-gp-accent/30 rounded-2xl shadow-2xl flex flex-col overflow-hidden" onClick={(event) => event.stopPropagation()}>
        <div className="flex items-center gap-3 px-4 py-3 border-b border-gp-accent/20 bg-gp-surface/30">
          <Button variant="ghost" size="sm" onClick={onClose} title="Back to Result" aria-label="Back to Result">
            X
          </Button>
          <div className="flex-1 min-w-0">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-4 pb-6 space-y-2 gp-scrollbar">
          {game.messages.length === 0 && (<p className="text-gp-mint/30 text-sm text-center py-10">No messages were sent.</p>)}
          {game.messages.map((msg) => {
            const isLocal = msg.playerId === game.localPlayerId;
            return (<div key={msg.id} className={`flex flex-col ${isLocal ? 'items-end' : 'items-start'}`}>
                <span className="text-gp-mint/50 text-[10px] px-1 mb-0.5">{msg.playerName}</span>
                ...
                {msg.triggeredWord && (<span className="text-xs mt-0.5 px-1">
...
function getRanking(game) {
    const firstOutByPlayer = new Map();
    game.messages.forEach((msg) => {
        if (!msg.triggeredWord)
            return;
    ...
    game.players.forEach((player, order) => {
        ...
        if (player.isOut && elimination) {
            outs.push({
                ...
                order,
                finalWords: player.wordSlots.map((slot) => slot.finalWord).filter(Boolean),
            });
        }
        else {
            survivors.push({
    ...
    const outSorted = [...outs].sort((a, b) => {
        const first = game.messages.findIndex((m) => m === firstOutByPlayer.get(a.player.id));
        const second = game.messages.findIndex((m) => m === firstOutByPlayer.get(b.player.id));
        return first - second;
    });
    const sorted = [
