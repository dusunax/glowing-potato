...
import { onValue, push, ref, onDisconnect, get, runTransaction, set, remove, } from 'firebase/database';
import { doc, serverTimestamp, setDoc } from 'firebase/firestore';
import { pickWords } from '../data/words';
import { getFirestoreDb, getRealtimeDb, hasRealtimeDbConfig, } from '../lib/firebase';
// ---------------------------------------------------------------------------
// Constants
// ---------------------------------------------------------------------------
...
function hiddenWordSlots() {
    return [
        {
            candidates: ['***', '***', '***'],
            votesByWord: Array(CANDIDATES_PER_SLOT).fill(0),
            finalWord: null,
        },
    ];
}
function normalizeDbWordSlot(slot) {
    return {
        candidates: slot.candidates,
        finalWord: slot.finalWord ?? null,
        votesByWord: Array.isArray(slot.votesByWord)
            ? slot.votesByWord
            : Array(slot.candidates.length).fill(0),
    };
}
function buildWordSlots(allAssigned) {
...
function voteCountsForSlot(slot, slotVotes) {
    const votesByWord = Array(slot.candidates.length).fill(0);
    if (!slotVotes)
        return votesByWord;
...
function resolveWordSlot(slot, votesByWord) {
...
function getUniqueOutOrder(messages) {
...
function buildGameHistory(game) {
    return {
...
function findTriggeredWordFromState(players, playerId, text) {
    const player = players.find((p) => p.id === playerId);
    if (!player)
        return null;
    for (const slot of player.wordSlots) {
        if (slot.finalWord && containsWord(text, slot.finalWord))
            return slot.finalWord;
    }
    return null;
}
function normalizeRoomSummary(rooms) {
...
function derivePlayerRows(roomPlayers, votes) {
    if (!roomPlayers)
        return [];
    return Object.entries(roomPlayers).map(([id, player]) => {
        const filteredVotes = Object.fromEntries(Object.entries(votes?.[id] ?? {}).map(([slotIdx, slotVotes]) => [
            slotIdx,
            Object.fromEntries(Object.entries(slotVotes).filter(([voterId]) => voterId in roomPlayers)),
        ]));
...
function resolveAllPlayerSlots(players, votes) {
    const nextPlayers = {};
    Object.entries(players).forEach(([id, player]) => {
        const filteredVotes = Object.fromEntries(Object.entries(votes?.[id] ?? {}).map(([slotIdx, slotVotes]) => [
            slotIdx,
            Object.fromEntries(Object.entries(slotVotes).filter(([voterId]) => voterId in players)),
        ]));
...
export function useDontSayIt() {
    ...
    const hostVotingTimerRef = useRef(null);
    const hostGameTimerRef = useRef(null);
    const roomUnsubRef = useRef(null);
    const roomListUnsubRef = useRef(null);
    const onDisconnectRef = useRef(null);
    const roomCleanupDisconnectRef = useRef(null);
    const finishedGameArchiveRef = useRef(new Set());
    useEffect(() => {
        if (!hasRealtimeDbConfig() || !db)
            return;
    ...
    const stopHostTimers = useCallback(() => {
        if (hostVotingTimerRef.current)
            clearInterval(hostVotingTimerRef.current);
        if (hostGameTimerRef.current)
            clearInterval(hostGameTimerRef.current);
        hostVotingTimerRef.current = null;
        hostGameTimerRef.current = null;
    }, []);
    const setDisconnectCleanup = useCallback(async (roomId, playerId) => {
        if (!db)
            return;
    ...
    const setRoomCleanupDisconnect = useCallback(async (roomId) => {
        if (!db)
            return;
        if (roomCleanupDisconnectRef.current) {
            try {
    ...
    const upsertUserProfile = useCallback(async (playerId, playerName, roomId) => {
        if (!firestoreDb || !playerId)
            return;
    ...
    const archiveFinishedGame = useCallback(async (targetGame) => {
        if (!targetGame?.roomId)
            return;
        if (finishedGameArchiveRef.current.has(targetGame.roomId))
            return;
        finishedGameArchiveRef.current.add(targetGame.roomId);
        setRooms((prev) => prev.filter((room) => room.id !== targetGame.roomId));
        if (firestoreDb) {
            const historyDoc = doc(firestoreDb, FIRESTORE_HISTORY_COLLECTION, FIRESTORE_HISTORY_GAME_TYPE, FIRESTORE_HISTORY_DOC_COLLECTION, targetGame.roomId);
    ...
    useEffect(() => {
        ...
        roomUnsubRef.current = onValue(roomRef, (snapshot) => {
            ...
            const isFinalHost = !!(localPlayerId &&
            ...
            if (payload.hostId && !playersMap[payload.hostId]) {
                void runTransaction(roomRef, (current) => {
                    ...
                    if (hostId && current.players[hostId])
                        return current;
    ...
    const startHostVotingTimer = useCallback(() => {
        if (!db || !localRoomId || !localPlayerId || hostVotingTimerRef.current)
            return;
        const roomRef = ref(db, roomPath(localRoomId));
        hostVotingTimerRef.current = setInterval(async () => {
            await runTransaction(roomRef, (current) => {
                if (!current || current.phase !== 'voting' || current.hostId !== localPlayerId)
                    return current;
    ...
    const startHostGameTimer = useCallback(() => {
        if (!db || !localRoomId || !localPlayerId || hostGameTimerRef.current)
            return;
        const roomRef = ref(db, roomPath(localRoomId));
        hostGameTimerRef.current = setInterval(async () => {
            await runTransaction(roomRef, (current) => {
                if (!current || current.phase !== 'playing' || current.hostId !== localPlayerId)
                    return current;
    ...
    const createRoom = useCallback(async (title, visibility, maxPlayers, playerName) => {
        if (!db)
            return;
    ...
    const addOrUpdatePlayerInRoom = useCallback(async (roomId, playerName) => {
        if (!db || !roomId)
            return false;
        const playerId = localPlayerStorageId();
        const normalizedName = sanitizePlayerName(playerName);
        const roomRef = ref(db, roomPath(roomId));
        const joinResult = await runTransaction(roomRef, (current) => {
            if (!current)
                return current;
    ...
    const joinPrivateRoom = useCallback(async (roomId, playerName) => {
        if (!db)
            return 'not-found';
    ...
    const leaveRoom = useCallback(async () => {
        ...
        await runTransaction(ref(db, roomPath(localRoomId)), (current) => {
            if (!current)
                return current;
            const players = { ...current.players };
    ...
    const castVote = useCallback(async (targetPlayerId, slotIndex, wordIndex) => {
        if (!db || !localRoomId || !localPlayerId || !game || game.phase !== 'voting')
            return;
        if (!Number.isInteger(wordIndex) || wordIndex < 0 || wordIndex >= CANDIDATES_PER_SLOT)
            return;
        if (!Number.isInteger(slotIndex) || slotIndex < 0)
            return;
        if (targetPlayerId === localPlayerId)
            return;
        const voteRef = ref(db, `${roomPath(localRoomId)}/votes/${targetPlayerId}/${slotIndex}/${localPlayerId}`);
        await set(voteRef, wordIndex);
    }, [db, game, localPlayerId, localRoomId]);
    const sendMessage = useCallback(async (text) => {
        if (!db || !localRoomId || !localPlayerId || !game || game.phase !== 'playing')
            return;
        ...
        if (!triggeredWord)
            return;
        const result = await runTransaction(ref(db, roomPath(localRoomId)), (current) => {
