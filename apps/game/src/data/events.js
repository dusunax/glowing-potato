...
export const COLLECT_MESSAGES = {
    success: (name, emoji) => `You found a ${emoji} ${name}!`,
    empty: () => 'Nothing to collect right now. Try a different time or weather.',
};
export const CRAFT_MESSAGES = {
    success: (name) => `✨ Crafted: ${name}!`,
    noIngredients: (name) => `Not enough ingredients for ${name}.`,
};
export const TIME_MESSAGES = {
    newDay: (day) => `🌅 Day ${day} begins.`,
    seasonChange: (season) => `🍃 The season changed to ${season}!`,
};
