// Static item definitions. No logic here — pure data only.
// To add a new item: copy an entry and adjust id, name, emoji, rarity, category, spawnConditions.

import type { Item } from '../types/items';

export const ITEMS: Item[] = [
  // ── Animal loot (not naturally spawnable) ─────────────────────────────────
  {
    id: 'raw_meat',
    name: 'Raw Meat',
    emoji: '🥩',
    description: 'Fresh meat dropped by a defeated animal.',
    rarity: 1,
    category: 'creature',
    tags: ['loot'],
    spawnConditions: { seasons: [] }, // loot-only, never foraged
  },
  {
    id: 'branch',
    name: 'Branch',
    emoji: '🪵',
    description: 'A sturdy branch useful for basic crafting.',
    rarity: 1,
    category: 'crafted',
    tags: ['craft-base', 'fuel', 'base-resource'],
    spawnConditions: { biomes: ['mountain', 'forest'] },
  },
  {
    id: 'wild_root',
    name: 'Wild Root',
    emoji: '🌿',
    description: 'A hardy root pulled from the soil, good for basic preparations.',
    rarity: 1,
    category: 'flora',
    tags: ['food', 'craft-base', 'base-resource'],
    healingAmount: 1,
    spawnConditions: {},
  },
  {
    id: 'drift_wood',
    name: 'Drift Wood',
    emoji: '🪵',
    description: 'Weathered driftwood washed ashore for improvised tools.',
    rarity: 1,
    category: 'mineral',
    tags: ['craft-base', 'base-resource'],
    spawnConditions: {
      biomes: ['beach', 'swamp'],
    },
  },
  {
    id: 'wooden_sword',
    name: 'Wooden Sword',
    emoji: '🗡️',
    description: 'A sharpened branch tied with twine for quick, basic combat.',
    rarity: 2,
    category: 'weapon',
    tags: ['weapon', 'melee'],
    attackPower: 2,
    spawnConditions: { seasons: [] }, // crafted-only
  },
  {
    id: 'iron_knife',
    name: 'Iron Knife',
    emoji: '🗡️',
    description: 'A forged iron knife with a sharper edge for cleaner kills.',
    rarity: 4,
    category: 'weapon',
    tags: ['weapon', 'melee'],
    attackPower: 6,
    spawnConditions: { seasons: [] }, // reward-only
  },
  {
    id: 'stone_blade',
    name: 'Stone Blade',
    emoji: '🗡️',
    description: 'A blade carved from hard stone and tied to a branch handle.',
    rarity: 3,
    category: 'weapon',
    tags: ['weapon', 'melee'],
    attackPower: 4,
    spawnConditions: {
      biomes: ['everywhere'],
    },
  },
  {
    id: 'bone_spear',
    name: 'Bone Spear',
    emoji: '🔱',
    description: 'A hunting spear with a sharpened bone point.',
    rarity: 4,
    category: 'weapon',
    tags: ['weapon', 'melee'],
    attackPower: 4,
    spawnConditions: {
      biomes: ['everywhere'],
    },
  },
  {
    id: 'grilled_meat',
    name: 'Grilled Meat',
    emoji: '🍖',
    description: 'Fresh game meat seared over an open flame.',
    rarity: 2,
    category: 'creature',
    tags: ['food', 'cooking'],
    healingAmount: 3,
    xpGain: 2,
    spawnConditions: { seasons: [] }, // crafted-only
  },
  {
    id: 'animal_hide',
    name: 'Animal Hide',
    emoji: '🪶',
    description: 'Tough hide stripped from a defeated animal.',
    rarity: 1,
    category: 'creature',
    tags: ['material', 'loot'],
    spawnConditions: { seasons: [] }, // loot-only, never foraged
  },
  {
    id: 'bone',
    name: 'Bone',
    emoji: '🦴',
    description: 'A sturdy bone reclaimed from fallen skeletons.',
    rarity: 1,
    category: 'creature',
    tags: ['material', 'loot'],
    spawnConditions: { seasons: [] }, // loot-only, never foraged
  },
  {
    id: 'hunter_stew',
    name: 'Hunter Stew',
    emoji: '🍲',
    description: 'A hearty stew made from fresh game and wild mushrooms.',
    rarity: 2,
    category: 'creature',
    tags: ['food', 'cooking'],
    healingAmount: 5,
    xpGain: 4,
    spawnConditions: { seasons: [] },
  },
  {
    id: 'gold_chunk',
    name: 'Gold ⚜️',
    emoji: '⚜️',
    description: 'A ceremonial gold charm from the treasure chest.',
    rarity: 2,
    category: 'mineral',
    tags: ['loot'],
    spawnConditions: { seasons: [] },
  },
  {
    id: 'healing_potion',
    name: 'Healing Potion',
    emoji: '🧪',
    description: 'A sweet berry tonic that restores a lot of health.',
    rarity: 3,
    category: 'crafted',
    tags: ['potion'],
    healingAmount: 8,
    xpGain: 6,
    spawnConditions: { seasons: [] },
  },
  // ── Flora ──────────────────────────────────────────────────────────────────
  {
    id: 'sunflower',
    name: 'Sunflower',
    emoji: '🌻',
    description: 'Bright petals that absorb summer sunlight.',
    rarity: 1,
    category: 'flora',
    tags: ['flower', 'dye'],
    spawnConditions: {
      seasons: ['Spring', 'Summer'],
      weathers: ['Sunny'],
      timePeriods: ['Morning', 'Afternoon'],
    },
  },
  {
    id: 'apple',
    name: 'Apple',
    emoji: '🍎',
    description: 'A crisp apple from wild orchard trees.',
    rarity: 1,
    category: 'flora',
    tags: ['fruit', 'food'],
    healingAmount: 1,
    spawnConditions: {
      seasons: ['Spring', 'Summer'],
      weathers: ['Sunny'],
      timePeriods: ['Morning', 'Afternoon'],
    },
  },
  {
    id: 'herb',
    name: 'Herb',
    emoji: '🌿',
    description: 'A fragrant wild herb with a strong flavor.',
    rarity: 1,
    category: 'flora',
    tags: ['herb', 'cooking-ingredient'],
    healingAmount: 1,
    spawnConditions: {
      seasons: ['Spring', 'Summer', 'Autumn'],
      weathers: ['Sunny', 'Cloudy', 'Rainy'],
      timePeriods: ['Morning', 'Afternoon', 'Evening'],
    },
  },
  {
    id: 'morning_dew_flower',
    name: 'Morning Dew Flower',
    emoji: '🌸',
    description: 'A delicate flower that blooms only in dawn mist.',
    rarity: 2,
    category: 'flora',
    tags: ['flower', 'potion-ingredient'],
    spawnConditions: {
      seasons: ['Spring'],
      timePeriods: ['Morning'],
      weathers: ['Foggy', 'Cloudy'],
    },
  },
  {
    id: 'mushroom',
    name: 'Mushroom',
    emoji: '🍄',
    description: 'Pops up after heavy rains in foggy conditions.',
    rarity: 1,
    category: 'flora',
    tags: ['mushroom', 'food'],
    healingAmount: 2,
    spawnConditions: {
      seasons: ['Autumn'],
      weathers: ['Rainy', 'Foggy'],
    },
  },
  {
    id: 'rain_lily',
    name: 'Rain Lily',
    emoji: '🌺',
    description: 'Blooms brilliantly right after rainfall.',
    rarity: 2,
    category: 'flora',
    tags: ['flower', 'potion-ingredient'],
    spawnConditions: {
      weathers: ['Rainy'],
      timePeriods: ['Afternoon', 'Evening'],
    },
  },
  {
    id: 'autumn_leaf',
    name: 'Autumn Leaf',
    emoji: '🍂',
    description: 'A golden-red leaf from a turning tree.',
    rarity: 1,
    category: 'flora',
    tags: ['leaf', 'dye'],
    spawnConditions: {
      seasons: ['Autumn'],
    },
  },
  {
    id: 'frostbloom',
    name: 'Frostbloom',
    emoji: '❄️',
    description: 'Rare ice-crystal flower found in deep winter.',
    rarity: 3,
    category: 'flora',
    tags: ['flower', 'crystal', 'potion-ingredient'],
    spawnConditions: {
      seasons: ['Winter'],
      weathers: ['Snowy'],
      timePeriods: ['Night'],
    },
  },
  {
    id: 'sunberry',
    name: 'Sunberry',
    emoji: '🍓',
    description: 'Sweet red berries ripened by summer sun.',
    rarity: 1,
    category: 'flora',
    tags: ['berry', 'food'],
    healingAmount: 1,
    spawnConditions: {
      seasons: ['Summer', 'Spring'],
      weathers: ['Sunny'],
      timePeriods: ['Afternoon'],
    },
  },
  {
    id: 'winter_berry',
    name: 'Winter Berry',
    emoji: '🫐',
    description: 'Icy blue berries that survive frost.',
    rarity: 2,
    category: 'flora',
    tags: ['berry', 'food'],
    healingAmount: 1,
    spawnConditions: {
      seasons: ['Winter'],
      weathers: ['Snowy', 'Cloudy'],
    },
  },
  {
    id: 'four_leaf_clover',
    name: 'Four-Leaf Clover',
    emoji: '🍀',
    description: 'Hard to find but brings good luck.',
    rarity: 3,
    category: 'flora',
    tags: ['lucky', 'special'],
    spawnConditions: {
      seasons: ['Spring'],
      weathers: ['Sunny', 'Cloudy'],
      timePeriods: ['Morning'],
    },
  },
  {
    id: 'moongrass',
    name: 'Moongrass',
    emoji: '🌿',
    description: 'Luminescent grass that glows under moonlight.',
    rarity: 2,
    category: 'flora',
    tags: ['grass', 'glow'],
    spawnConditions: {
      timePeriods: ['Night'],
    },
  },
  {
    id: 'fern_spore',
    name: 'Fern Spore',
    emoji: '🍀',
    description: 'A tiny spore bundle for high-value brews and infusions.',
    rarity: 2,
    category: 'flora',
    tags: ['craft-base', 'base-resource'],
    spawnConditions: {
      seasons: ['Spring', 'Summer', 'Autumn'],
      timePeriods: ['Morning', 'Afternoon', 'Evening'],
      biomes: ['forest', 'swamp'],
    },
  },
  {
    id: 'pinecone',
    name: 'Pinecone',
    emoji: '🌲',
    description: 'Falls from pines in the cool season.',
    rarity: 1,
    category: 'flora',
    tags: ['seed', 'craft-base'],
    spawnConditions: {
      seasons: ['Autumn', 'Winter'],
    },
  },
  // ── Minerals ───────────────────────────────────────────────────────────────
  {
    id: 'river_stone',
    name: 'River Shingle',
    emoji: '🪨',
    description: 'A smooth piece of stone shaped by flowing water.',
    rarity: 1,
    category: 'mineral',
    tags: ['stone', 'craft-base', 'base-resource'],
    spawnConditions: {
      biomes: ['beach', 'lake'],
    },
  },
  {
    id: 'stone',
    name: 'Stone',
    emoji: '🪨',
    description: 'A rough stone fragment found in rocky areas.',
    rarity: 1,
    category: 'mineral',
    tags: ['stone', 'craft-base', 'base-resource'],
    spawnConditions: {
      biomes: ['cave'],
    },
  },
  {
    id: 'ash_chip',
    name: 'Ash Chip',
    emoji: '🪨',
    description: 'Compact ash with trace heat, useful for reinforcing tools.',
    rarity: 3,
    category: 'mineral',
    tags: ['craft-base', 'base-resource'],
    spawnConditions: {
      biomes: ['desert', 'rock'],
      weathers: ['Sunny'],
      timePeriods: ['Afternoon'],
    },
  },
  {
    id: 'ember_root',
    name: 'Ember Root',
    emoji: '🔥',
    description: 'A warming root that appears in dry, open biomes.',
    rarity: 4,
    category: 'flora',
    tags: ['food', 'craft-base', 'cooking-ingredient'],
    healingAmount: 2,
    spawnConditions: {
      biomes: ['desert', 'mountain', 'rock'],
      weathers: ['Sunny', 'Cloudy'],
    },
  },
  {
    id: 'void_moss',
    name: 'Void Moss',
    emoji: '🌑',
    description: 'A pitch-black moss found only in deep darkness.',
    rarity: 5,
    category: 'flora',
    tags: ['craft-base', 'magic', 'base-resource'],
    spawnConditions: {
      biomes: ['cave'],
      weathers: ['Foggy', 'Cloudy', 'Snowy'],
      timePeriods: ['Night'],
    },
  },
  {
    id: 'clam',
    name: 'Clam',
    emoji: '🐚',
    description: 'A sea-dwelling clam with a hard shell and a slimy surprise inside.',
    rarity: 1,
    category: 'mineral',
    tags: ['ocean', 'food', 'craft-base'],
    spawnConditions: {
      biomes: ['beach'],
    },
  },
  {
    id: 'raindrop',
    name: 'Raindrop Vial',
    emoji: '💧',
    description: 'Pure rain collected in a tiny vial.',
    rarity: 1,
    category: 'mineral',
    tags: ['liquid', 'craft-base'],
    spawnConditions: {
      weathers: ['Rainy'],
    },
  },
  {
    id: 'moonstone',
    name: 'Moonstone',
    emoji: '💎',
    description: 'A shimmering gem found under moonlight.',
    rarity: 3,
    category: 'mineral',
    tags: ['gem', 'magic'],
    spawnConditions: {
      timePeriods: ['Night'],
    },
  },
  {
    id: 'sunstone',
    name: 'Sunstone',
    emoji: '🌟',
    description: 'A warm golden crystal found on sunny summer afternoons.',
    rarity: 3,
    category: 'mineral',
    tags: ['gem', 'magic'],
    spawnConditions: {
      seasons: ['Summer'],
      weathers: ['Sunny'],
      timePeriods: ['Afternoon'],
    },
  },
  {
    id: 'fog_pearl',
    name: 'Fog Pearl',
    emoji: '🫧',
    description: 'A mysterious orb that appears in heavy fog.',
    rarity: 2,
    category: 'mineral',
    tags: ['crystal', 'magic'],
    spawnConditions: {
      weathers: ['Foggy'],
      timePeriods: ['Morning', 'Evening'],
    },
  },
  {
    id: 'storm_shard',
    name: 'Storm Shard',
    emoji: '⚡',
    description: 'A charged crystal fragment from a thunderstorm.',
    rarity: 3,
    category: 'mineral',
    tags: ['crystal', 'magic', 'energy'],
    spawnConditions: {
      weathers: ['Rainy'],
      timePeriods: ['Evening', 'Night'],
    },
  },
  {
    id: 'clam_soup',
    name: 'Clam Soup',
    emoji: '🍲',
    description: 'Hearty soup made from fresh clams and a pinch of rainwater.',
    rarity: 2,
    category: 'creature',
    tags: ['food', 'cooking'],
    healingAmount: 4,
    xpGain: 3,
    spawnConditions: {
      seasons: [],
    },
  },
  {
    id: 'mushroom_soup',
    name: 'Mushroom Soup',
    emoji: '🍲',
    description: 'Warm soup made from freshly picked mushrooms and clean water.',
    rarity: 2,
    category: 'creature',
    tags: ['food', 'cooking'],
    healingAmount: 3,
    xpGain: 3,
    spawnConditions: {
      seasons: [],
    },
  },
  {
    id: 'apple_herb_salad',
    name: 'Apple Herb Salad',
    emoji: '🥗',
    description: 'A fresh salad from wild apple, sunberry, and herbs.',
    rarity: 2,
    category: 'creature',
    tags: ['food', 'cooking'],
    healingAmount: 3,
    xpGain: 2,
    spawnConditions: {
      seasons: [],
    },
  },
  // ── Creatures ──────────────────────────────────────────────────────────────
  {
    id: 'firefly',
    name: 'Firefly',
    emoji: '✨',
    description: 'A tiny glowing insect of warm summer nights.',
    rarity: 2,
    category: 'creature',
    tags: ['insect', 'glow'],
    spawnConditions: {
      seasons: ['Summer'],
      timePeriods: ['Evening', 'Night'],
    },
  },
  {
    id: 'spring_butterfly',
    name: 'Spring Butterfly',
    emoji: '🦋',
    description: 'Flutters through spring meadows on sunny days.',
    rarity: 2,
    category: 'creature',
    tags: ['insect', 'beauty'],
    spawnConditions: {
      seasons: ['Spring'],
      weathers: ['Sunny', 'Cloudy'],
      timePeriods: ['Morning', 'Afternoon'],
    },
  },
  {
    id: 'night_moth',
    name: 'Night Moth',
    emoji: '🌙',
    description: 'A dusty moth drawn to moonlight.',
    rarity: 1,
    category: 'creature',
    tags: ['insect', 'night'],
    spawnConditions: {
      timePeriods: ['Night'],
    },
  },
  {
    id: 'frost_sprite',
    name: 'Frost Sprite',
    emoji: '🧊',
    description: 'A tiny ice elemental seen only in deep winter midnight.',
    rarity: 4,
    category: 'creature',
    tags: ['elemental', 'magic'],
    spawnConditions: {
      seasons: ['Winter'],
      weathers: ['Snowy'],
      timePeriods: ['Night'],
    },
  },
  // ── Special ────────────────────────────────────────────────────────────────
  {
    id: 'ancient_seed',
    name: 'Ancient Seed',
    emoji: '🌱',
    description: 'A mysterious seed of unknown origin.',
    rarity: 3,
    category: 'special',
    tags: ['seed', 'mystery'],
    spawnConditions: {
      seasons: ['Spring', 'Autumn'],
    },
  },
  {
    id: 'star_fragment',
    name: 'Star Fragment',
    emoji: '💫',
    description: 'A piece of fallen star. Extraordinarily rare.',
    rarity: 4,
    category: 'special',
    tags: ['celestial', 'magic', 'tier-3'],
    spawnConditions: {
      timePeriods: ['Night'],
      weathers: ['Cloudy', 'Snowy'],
    },
  },
];

export const getItemById = (id: string): Item | undefined =>
  ITEMS.find((item) => item.id === id);
