...
export function useGameState() {
    ...
    const [defeatedAnimals, setDefeatedAnimals] = useState([]);
    const pushEvent = useCallback((message, type = 'info') => {
        const ev = { id: String(++eventCounter), message, type, timestamp: Date.now() };
        setEvents((prev) => [...prev, ev].slice(-MAX_EVENT_LOGS));
    }, []);
    const xpNeededForNextLevel = useCallback((level) => XP_BASE_TO_NEXT_LEVEL + (level - 1) * XP_LEVEL_GROWTH, []);
    const gainExperience = useCallback((amount) => {
        if (amount <= 0 || isPlayerDead)
            return;
    ...
    const { position, currentBiomeInfo, visitedTiles, knownTiles, canMoveTo, isAdjacent, moveTo, consumeTileResource, getTileResources, replenishTileResources, getReachableTiles, getPathLength, MAP_ROWS, MAP_COLS, } = useMap();
    const { hand, selectedCard, selectCard, playCard, deckSize } = useActionCards();
    const { animals, moveAnimals, spawnCaveWave, attackAnimal, getAnimalsAt, getAdjacentHostile, getAdjacentAnimals, } = useAnimals();
    ...
    const advanceTimeBySteps = useCallback((steps) => advance(steps), [advance]);
    const handleCraft = useCallback((recipeId) => {
    ...
    const postMoveEffects = useCallback((newPosition, timeSteps = 1) => {
        // Advance time and resolve related effects.
        const { messages, isNewDay, resourceRefillCount, caveSpawnCount, } = advanceTimeBySteps(timeSteps);
        ...
        const adjacentHostiles = nextAnimals.filter((a) => a.alive &&
            a.behavior === 'hostile' &&
     ...
     */
    const handlePlayCard = useCallback((card, moveTarget) => {
        if (isPlayerDead)
            return;
        switch (card.type) {
            case 'forage': {
    ...
    const handleStrike = useCallback((animalId) => {
        if (isPlayerDead)
            return;
        ...
        if (message)
            pushEvent(message, defeated ? 'success' : 'warning');
        if (defeated) {
