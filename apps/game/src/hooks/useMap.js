...
import { MAP_GRID, MAP_ROWS, MAP_COLS, INITIAL_PLAYER_POSITION, BIOME_INFO, getMazeNeighbors, hasPassage, } from '../data/map';
...
export function useMap() {
    const [position, setPosition] = useState(INITIAL_PLAYER_POSITION);
    // Fog of war: track which tiles the player has visited
    const [visitedTiles, setVisitedTiles] = useState(() => new Set([tileKey(INITIAL_PLAYER_POSITION.x, INITIAL_PLAYER_POSITION.y)]));
    // Resource caps are fixed per tile (1–3 per tile), and current resources are tracked separately.
    const tileResourceCapsRef = useRef(buildInitialResources());
    // Per-tile resource values are stored in a ref for synchronous reads
    // and shadowed in state to drive re-renders.
    const tileResourcesRef = useRef({ ...tileResourceCapsRef.current });
    ...
    const getReachableTiles = useCallback((from, maxSteps) => {
        const reachable = new Set();
        const queue = [{ pos: from, steps: 0 }];
        const seen = new Set([tileKey(from.x, from.y)]);
        while (queue.length > 0) {
            const { pos, steps } = queue.shift();
            if (steps > 0)
                reachable.add(tileKey(pos.x, pos.y));
            if (steps < maxSteps) {
     ...
     */
    const getPathLength = useCallback((from, to) => {
        if (from.x < 0 || from.x >= MAP_COLS || from.y < 0 || from.y >= MAP_ROWS ||
    ...
    const canMoveTo = useCallback((x, y, range = 1) => {
    ...
    const isAdjacent = useCallback((x, y) => hasPassage(position.x, position.y, x, y), [position]);
    const moveTo = useCallback((x, y) => {
        if (getPathLength(position, { x, y }) === -1)
            return false;
    ...
    const consumeTileResource = useCallback((x, y) => {
    ...
    const getTileResources = useCallback((x, y) => tileResourcesRef.current[tileKey(x, y)] ?? 0, []);
