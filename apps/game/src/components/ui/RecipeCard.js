...
export function RecipeCard({ recipe, canCraft, onCraft, getQuantity }) {
    const resultItem = ITEMS.find((i) => i.id === recipe.result.itemId);
    return (<div className={`bg-gp-bg/30 border rounded-lg p-4 transition-colors ${canCraft ? 'border-gp-mint/40' : 'border-gp-accent/20'}`}>
      <div className="flex items-start justify-between gap-2 mb-2">
        ...
        {resultItem && (<span className="text-2xl" title={resultItem.name}>{resultItem.emoji}</span>)}
      </div>
      <div className="flex flex-wrap gap-2 mb-3">
        {recipe.ingredients.map((ing) => {
            ...
            return (<span key={ing.itemId} className={`text-xs px-2 py-1 rounded-full border ${enough
      ...
      <Button variant={canCraft ? 'primary' : 'ghost'} size="sm" onClick={onCraft} disabled={!canCraft} className="w-full justify-center">
