...
export function ConditionsPanel({ conditions, onAdvanceTime, onCollect, events }) {
    ...
    return (<div className="bg-gp-surface border border-gp-accent/30 rounded-xl p-4">
      ...
      {events.length > 0 && (<div className="space-y-1">
          {events.slice(0, 3).map((ev) => (<div key={ev.id} className={`text-sm px-3 py-1 rounded-md ${ev.type === 'success' ? 'bg-gp-accent/20 text-gp-mint' :
