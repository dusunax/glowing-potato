...
export function MapPanel({ position, selectedCard, showPlayerMoveHint = false, onTileClick, currentBiomeInfo, visitedTiles, knownTiles, getTileResources, getAnimalsAt, getReachableTiles, nearbyAnimalTiles, }) {
    ...
    const reachable = isMoveCard
        ...
        : new Set();
    return (<div className="bg-gp-surface border border-gp-accent/30 rounded-xl p-4">
      ...
      {/* World grid */}
      <div className="grid gap-1 mb-3" style={{ gridTemplateColumns: `repeat(${MAP_COLS}, 1fr)` }}>
        {MAP_GRID.map((row, y) => row.map((biome, x) => {
            ...
            // Build tile class
            let tileClass = 'aspect-square flex w-full h-full flex-col items-center justify-center rounded-lg text-base transition-all duration-150 relative select-none';
            if (isReachable) {
                ...
                tileClass += ` border-2 border-gp-mint bg-gp-accent/40 ring-2 ring-gp-mint shadow-lg z-10 ${showPlayerMoveHint ? 'animate-pulse scale-105 ring-gp-mint/90' : ''}`;
            }
            else if (!isVisited && isKnown) {
            ...
            return (<div key={key} className="relative">
                <button onClick={() => clickable ? onTileClick(x, y) : undefined} disabled={!clickable} className={tileClass} title={hidden ? '???' : biomeInfo.name} aria-label={hidden ? 'Unknown tile' : `${biomeInfo.name}${isPlayer ? ' (you are here)' : ''}`}>
                  {hidden ? (<>
                      <span className="text-gp-accent/30 text-lg">?</span>
                      {animals.length > 0 && (<span className="text-[9px] leading-none opacity-50">🐾</span>)}
                    </>) : (<>
                      {/* Biome emoji */}
                      ...
                      {animals.length > 0 && (<span className={`text-[9px] leading-none${isKnown && !isVisited ? ' opacity-50' : ''}`}>
                          {animals[0].emoji}{animals.length > 1 ? `+${animals.length - 1}` : ''}
                        </span>)}

                      {/* Resource counter (small badge bottom-right) */}
                      {isVisited && (<span className={[
                ...
                {x < MAP_COLS - 1 && hasRightPassage && !hidden && (<div className="absolute right-[-4px] top-1/2 -translate-y-1/2 w-1.5 h-2 bg-gp-accent/30 z-10 rounded-sm pointer-events-none"/>)}
                {/* Down passage indicator */}
                {y < MAP_ROWS - 1 && hasDownPassage && !hidden && (<div className="absolute bottom-[-4px] left-1/2 -translate-x-1/2 h-1.5 w-2 bg-gp-accent/30 z-10 rounded-sm pointer-events-none"/>)}
              </div>);
        }))}
      </div>

      <div className='h-4'>
        {isMoveCard && (<p className="mt-2 text-xs text-gp-mint/70 text-center animate-pulse">
            ✨ Click a highlighted tile to move there
          </p>)}
        {isMoveCard && nearbyAnimalTiles.size > 0 && (<p className="mt-2 text-xs text-gp-mint/70 text-center animate-pulse">
            🐾 Adjacent animals are visible in the list
          </p>)}
        {!isMoveCard && nearbyAnimalTiles.size > 0 && (<p className="mt-2 text-xs text-red-300/90 text-center animate-pulse">
