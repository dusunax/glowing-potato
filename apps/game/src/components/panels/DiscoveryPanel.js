...
export function DiscoveryPanel({ discovered }) {
    return (<div className="flex flex-col">
      ...
      <div className="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-6 gap-2 overflow-y-auto">
        {ITEMS.map((item) => {
            ...
            return (<div key={item.id} className={`rounded-lg p-2 border transition-all ${found
              ...
              {found && (<Badge label={item.rarity} variant={RARITY_BADGE_MAP[item.rarity] ?? 'muted'} className="mt-1"/>)}
            </div>);
        })}
      </div>
    </div>);
