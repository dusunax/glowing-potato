...
export function InventoryPanel({ inventory, beltSlots, selectedBeltSlot, onSelectBeltSlot, onAssignToBelt, onClearBeltSlot, canUseInBelt, }) {
    const EMPTY_SLOT_CLASS = 'min-h-[74px] border border-dashed border-gp-mint/55 rounded-lg';
      ...
      <div className="mb-3 space-y-2">
        ...
        <div className="grid grid-cols-8 gap-2">
          {beltSlots.map((itemId, index) => {
            ...
            if (!item) {
                return (<button key={`belt-slot-config-${index}`} type="button" onClick={() => onSelectBeltSlot(index)} className={`relative w-full min-h-12 rounded-lg border transition-all ${isSelected ? 'ring-2 ring-gp-mint/80 border-gp-mint bg-gp-mint/15 shadow-[0_0_0_2px_rgba(16,185,129,0.25)]' : 'border-gp-accent/30 bg-gp-bg/30 hover:border-gp-accent/70'}`}>
            ...
            return (<button key={`belt-slot-config-${index}`} type="button" onClick={() => onSelectBeltSlot(index)} className={`relative min-h-12 rounded-lg border transition-colors ${isSelected ? 'border-gp-mint bg-gp-mint/20 ring-2 ring-gp-mint/70' : 'border-gp-accent/30 bg-gp-bg/40 hover:border-gp-accent/70'} ${itemId ? 'text-gp-mint' : 'text-gp-mint/60'}`}>
      ...
      <div className="flex-1 min-h-0 flex flex-col gap-2">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 overflow-y-auto">
          {Array.from({ length: slots }).map((_, index) => {
            ...
            if (!slot) {
                return (<div key={`empty-${index}`} className={`${EMPTY_SLOT_CLASS} bg-gp-bg/10 flex items-center justify-center text-[11px] text-gp-mint/70 font-semibold uppercase tracking-wide`} style={EMPTY_SLOT_STYLE}>
                  Empty
                </div>);
            }
            const isBeltItem = canUseInBelt(slot.itemId);
            return (<div key={slot.itemId} className="rounded-lg border border-gp-accent/30 bg-gp-bg/30 p-2 space-y-2">
                <ItemCard slot={slot}/>
                <div className="flex items-center gap-2">
                  {isBeltItem && (<button type="button" onClick={() => onAssignToBelt(selectedBeltSlot, slot.itemId)} className="text-xs px-2 py-1 rounded bg-gp-accent/30 text-gp-mint hover:bg-gp-accent/50">
                      Assign to {selectedSlotLabel}
                    </button>)}
                  {isBeltItem && selectedSlotItem === slot.itemId && (<button type="button" onClick={() => {
