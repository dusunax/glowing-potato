...
export function GameLobby({ onSelectGame, user, nickname, onSignIn, onSignOut, onUpdateNickname }) {
    ...
    return (<div className="relative min-h-screen flex flex-col items-center justify-center p-6 bg-gp-bg">
      {/* Top-right auth controls */}
      <div className="absolute top-4 right-4 flex items-center gap-2">
        {user ? (<>
            <button type="button" onClick={() => setShowUserEdit(true)} aria-label="Edit profile" title={userDisplayName || 'Profile'} className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-gp-accent/30 text-gp-mint/70 hover:text-gp-mint hover:border-gp-accent/60 hover:bg-gp-accent/10 transition-colors text-sm">
            ...
            {onSignOut && (<button type="button" onClick={onSignOut} aria-label="Sign out" className="px-2 py-1.5 rounded-lg border border-gp-accent/20 text-gp-mint/50 hover:text-gp-mint/80 hover:border-gp-accent/40 transition-colors text-xs">
                Sign out
              </button>)}
          </>) : (onSignIn && (<button type="button" onClick={onSignIn} className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-gp-accent/30 text-gp-mint/70 hover:text-gp-mint hover:border-gp-accent/60 hover:bg-gp-accent/10 transition-colors text-sm font-medium">
              <span>G</span>
              Sign in
            </button>))}
      </div>
      ...
      {/* User edit popup */}
      {showUserEdit && user && onUpdateNickname && (<UserEditPopup currentNickname={userDisplayName} onSave={onUpdateNickname} onClose={() => setShowUserEdit(false)}/>)}
      {/* Header */}
      ...
      <div className="w-full max-w-3xl">
        ...
        <div className="relative flex items-center justify-center gap-4 mb-6 select-none">
          ...
          {/* Active card */}
          <div className={`flex-shrink-0 transition-opacity duration-150 ${spinning ? 'opacity-60' : 'opacity-100'}`}>
        ...
        <div className="flex justify-center gap-2 mb-6">
          {MINI_GAMES.map((_, i) => (<button key={i} onClick={() => !spinning && setActiveIndex(i)} className={`w-2 h-2 rounded-full transition-all duration-200 ${i === activeIndex
                ? 'bg-gp-mint scale-125'
                : 'bg-gp-accent/40 hover:bg-gp-accent'}`} aria-label={`Go to game ${i + 1}`}/>))}
        </div>

        {/* Controls */}
        <div className="flex justify-center items-center gap-3">
          <Button variant="secondary" size="icon" onClick={prev} disabled={spinning} aria-label="Previous game">
            ◀
          </Button>

          <Button variant="outline" size="lg" onClick={handleSpin} disabled={spinning}>
            {spinning ? '🎰 Spinning…' : '🎰 Spin'}
          </Button>

          <Button variant="secondary" size="icon" onClick={next} disabled={spinning} aria-label="Next game">
            ▶
          </Button>
        </div>
        ...
        <div className="flex justify-center mt-5">
          {active.status === 'available' ? (<Button variant="primary" size="lg" onClick={() => handlePlay(active)} className="min-w-48">
              ▶ Play {active.name}
            </Button>) : (
      ...
      <div className="mt-12 w-full max-w-3xl">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-3">
          {MINI_GAMES.map((game, i) => (<button key={game.id} onClick={() => !spinning && setActiveIndex(i)} className={`flex flex-col items-center gap-1.5 p-3 rounded-xl border transition-all duration-150 text-left ${i === activeIndex
                ? 'border-gp-mint/50 bg-gp-surface'
                : 'border-gp-accent/20 bg-gp-surface/30 hover:border-gp-accent/50 hover:bg-gp-surface/60'} ${game.status === 'coming-soon' ? 'opacity-50' : ''}`}>
              <span className="text-2xl">{game.emoji}</span>
              <span className="text-xs text-gp-mint font-medium text-center leading-tight">
                {game.name}
              </span>
              {game.status === 'coming-soon' && (<span className="text-[10px] text-gp-mint/50">Soon</span>)}
            </button>))}
        </div>
      </div>
      ...
      <div className="mt-12 w-full max-w-3xl pb-10">
        ...
        {leaderboardLoading ? (<div className="rounded-xl border-2 border-gp-accent/40 bg-gp-surface/90 overflow-hidden">
            {Array.from({ length: 10 }).map((_, index) => (<div key={`leaderboard-skeleton-${index}`} className={`flex items-center gap-3 px-4 py-3 text-sm border-b border-gp-accent/10 last:border-b-0 ${index === 0 ? 'bg-gp-bg/55' : 'bg-gp-surface/45'}`}>
          ...
          </div>) : (<div className="rounded-xl border-2 border-gp-accent/40 bg-gp-surface/90 overflow-hidden">
            {leaderboardRows.map((row) => {
                if (row.type === 'record') {
                    const record = row.record;
                    return (<div key={record.id ?? row.index} className={`flex items-center gap-3 px-4 py-3 text-sm border-b border-gp-accent/10 last:border-b-0 ${row.index === 0 ? 'bg-gp-bg/75' : 'bg-gp-surface/65'}`}>
                ...
                return (<div key={`empty-slot-${row.index}`} className={`flex items-center gap-3 px-4 py-3 text-sm border-b border-gp-accent/10 last:border-b-0 ${row.index === 0 ? 'bg-gp-bg/55' : 'bg-gp-surface/45'}`}>
        ...
        {!user && (<p className="text-center text-gp-mint/40 text-xs mt-3">
...
function GameCard({ game, active = false }) {
    return (<div className={`${CARD_WIDTH_CLASS} ${CARD_HEIGHT_CLASS} rounded-2xl p-6 flex flex-col items-center justify-center gap-3 border transition-colors duration-200 ${active
            ? 'bg-gp-surface border-gp-accent/60'
            : 'bg-gp-surface/50 border-gp-accent/20'}`} style={active
      ...
      {game.status === 'coming-soon' && (<Badge label="Coming Soon" variant="muted"/>)}
      {game.status === 'available' && active && (<Badge label="● Available" variant="success"/>)}
    </div>);
}
